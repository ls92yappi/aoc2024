...
func AddInput(key string, part string) {
	_, ok := Inputs[key]
	if !ok {
		Inputs[key] = []string{part}
	} else {
		Inputs[key] = append(Inputs[key], part)
	}
}
...
func processInput(fname string) string {
	...
	// construct expected Inputs and Outputs data types maps
	for k,g := range(Gates) {
		// ignore 00'th items, known good
		if (g.A=="x00" && g.B=="y00") || (g.A=="y00" && g.B=="x00") {
			continue
		}
		inputs := []string{g.A, g.B}
		sort.Strings(inputs)
		a := inputs[0]
		b := inputs[1]
		//   x01 -- 
		...
		//   D01 --
		switch {
		case string(a[0])=="x" && string(b[0])=="y":
			if g.F == "XOR" {
				Outputs[k] = "Diff"
			} else if g.F=="AND" {
				Outputs[k] = "And"
		...
		case g.F == "OR":
			...
			Outputs[k] = "Carry"
		default:
			AddInput(a, "Impossible")
			AddInput(b, "Impossible")
			Outputs[k] = "Impossible"
		}
	}

	// sort the input types for ease of access
	ikl := Inputs.SortedKeys()
	for _,k := range(ikl) {
		sl := Inputs[k]
		sort.Strings(sl)
		Inputs[k] = sl
	...
	// verify the outputs are fed the correct input types
	okl := Outputs.SortedKeys()
	for _,k := range(okl) {
		kPrefix := string(k[0])
		numInputs := len(Inputs[k])
		outk := Outputs[k]
		//   x01 -- 
		//         XOR -- D01 (rbr) --
		//   y01 --                  |
		...
		//   D01 --
		switch {
		case k=="z45" && Outputs[k]=="Carry":
			continue
		case numInputs==0 && kPrefix=="z" &&  outk=="Result":
			continue
		case numInputs==2 && Inputs[k][0]=="Forced" && Inputs[k][1]=="Result" && (outk=="Diff" || outk=="Carry"):
			continue
		case numInputs>0 && Inputs[k][0]=="Carry" && (outk=="And" || outk=="Forced"):
			continue
		default:
