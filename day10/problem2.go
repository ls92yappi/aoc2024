...
// route from trailhead t with index i at height h to goal g at height 9
func countAllRoutes(t, g Pos, i int, h int) {
	//fmt.Print(ShowAttempt(t,g,i,h))
	if (h == 9) {
		//fmt.Printf("Summit Check: t=%v g=%v h=%d\n", t, g, h)
		// found the goal
	...
	if nok { countAllRoutes(*n, g, i, up) }
	if sok { countAllRoutes(*s, g, i, up) }
	if eok { countAllRoutes(*e, g, i, up) }
	if wok { countAllRoutes(*w, g, i, up) }
